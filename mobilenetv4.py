...
class MobileNet(nn.Module):
    def __init__(self, class_nums=4):
        ...
        self.fc = nn.Linear(1280, class_nums)
        self.moire_fc = nn.Linear(1280, 1)
    ...
    def forward(self, x):
        ...
        x4 = self.layer4(x3)
        x5 = self.layer5(x4)
        x5 = torch.flatten(nn.functional.adaptive_avg_pool2d(x5, 1), 1)
        out = self.fc(x5)
        moire_out = self.moire_fc(x5)
        return torch.softmax(out, dim=1), torch.sigmoid(moire_out)
